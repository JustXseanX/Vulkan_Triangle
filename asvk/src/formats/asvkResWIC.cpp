...
static size_t WICBitsPerPixel( IWICImagingFactory* pFactory, const GUID& guid )
{
    ...
    asvk::RefPtr<IWICComponentInfo> info;
    ...
    asvk::RefPtr<IWICPixelFormatInfo> formatInfo;
...
//-------------------------------------------------------------------------------------------------
bool LoadResTextureFromWIC( const wchar_t* filename, asvk::ResTexture* pResult )
{
    ...
    static const uint32_t MaxTextureSize = 16384;    // D3D_FEATURE_LEVEL_11_0
    ...
    if ( width > MaxTextureSize || height > MaxTextureSize )
    {
        ...
        if ( width > height )
        {
            ...
            height = uint32_t( float( MaxTextureSize ) * ratio );
        }
        else
        {
            width  = uint32_t( float( MaxTextureSize ) / ratio );
    ...
    // ピクセルデータのメモリを確保.
    uint8_t* pPixels = new (std::nothrow) uint8_t [ slicePitch ];
    if ( !pPixels )
    { return false; }
    ...
    if ( 0 == memcmp( &convertGuid, &pixelFormat, sizeof(GUID) )
      ...
      && height == origHeight )
    {
        hr = frame->CopyPixels( 0, uint32_t( rowPitch ), uint32_t( slicePitch ), pPixels );
        if ( FAILED( hr ) )
        {
          ...
          || height != origHeight )
    {
        ...
        {
            hr = scaler->CopyPixels( 0, uint32_t( rowPitch ), uint32_t( slicePitch ), pPixels );
            if ( FAILED( hr ) )
            {
        ...
        else
        {
            ...
            hr = conv->CopyPixels( 0, uint32_t( rowPitch ), uint32_t( slicePitch ), pPixels );
            if ( FAILED( hr ) )
            {
    ...
    pRes->Width      = width;
    pRes->Height     = height;
    pRes->RowPitch   = uint32_t( rowPitch );
    pRes->SlicePitch = uint32_t( slicePitch );
    pRes->pPixels    = pPixels;

    // リソーステクスチャを設定.
    (*pResult).Dimension        = (height > 1) ? RESTEXTURE_DIMENSION_2D : RESTEXTURE_DIMENSION_1D;
    (*pResult).Width            = width;
